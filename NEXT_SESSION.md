...
## Current Status (2026-04-28, post-session #65)
- **NEON_DB_URL test failures RESOLVED**: Added `mock_usage_tracker_init` autouse fixture to `tests/conftest.py`. When `NEON_DB_URL` is absent, resets `_tracker_instance` singleton and mocks `UsageTracker.init_database` to a no-op. All 14 tests in `test_sms_mock.py` + `test_issue_52_template_mapping.py` now collect and run: 11 passed, 3 skipped-by-decorator, 0 failed. Committed `7c58ec4`. Closes yebyen/mecris#295.
- **Test suite state**: 860 passed, 0 failed (all prior `NEON_DB_URL` failures resolved). Suite is clean in headless CI.
...
- **yebyen/mecris ahead of kingdonb/mecris**: Commits `10be85d` (session #64, narrator presence skew fix) and `7c58ec4` (session #65, NEON_DB_URL test fix) not yet PRed.
...
- [x] **NEON_DB_URL test failures (session #65)**: `tests/conftest.py` `mock_usage_tracker_init` autouse fixture added. Root cause: `smart_send_message()` unconditionally calls `get_tracker()` → `UsageTracker()` → `init_database()` raises `EnvironmentError` when `NEON_DB_URL` absent. Fix: mock `init_database` to no-op and reset singleton per test. `PYTHONPATH=. python3 -m pytest tests/test_sms_mock.py tests/test_issue_52_template_mapping.py` → **11 passed, 3 skipped, 0 failed** (was 9 collection failures). Commit `7c58ec4`. **COMPLETE** — closes yebyen/mecris#295.
...
- [ ] **Open PR yebyen:main → kingdonb:main** for sessions #64+#65 commits (`10be85d` narrator presence skew fix, `7c58ec4` NEON_DB_URL test fix). Closes yebyen/mecris#294 and yebyen/mecris#295.
