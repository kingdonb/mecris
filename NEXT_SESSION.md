# Next Session: AI Framework Evaluation + scheduler test ordering fix

## Current Status (2026-04-30, post-session #82)
- **_start_leader_jobs test coverage (session #82)**: COMPLETE. 5 new tests added to `tests/test_presence_scheduler.py::TestSchedulerPresenceGuard`: idempotent (all jobs), idempotent (partial jobs), locked DB retry+success, locked DB exhaustion (4 sleeps), non-locked exception (immediate break). Commit `b7b2345`. Closes yebyen/mecris#316.
- **Full scheduler suite**: 52 tests across 5 files (test_presence_scheduler×21, test_scheduler_election×7, test_scheduler_jobs×24, test_scheduler_timer_reset×2[env-fail]) — all pass when run alphabetically (presence first). Note: test_scheduler_timer_reset.py requires actual `apscheduler` package (not present in CI env) — pre-existing.
- **Test ordering dependency**: `test_scheduler_jobs.py` requires `test_presence_scheduler.py` to run first (primes sys.modules with mocked psycopg2). Normal `pytest tests/` discovery (alphabetical) satisfies this. Isolated runs fail.
...
- [x] **_start_leader_jobs test coverage (session #82)**: 5 new tests in `TestSchedulerPresenceGuard` (idempotent-all, idempotent-partial, locked-retry-success, locked-exhausted, non-lock-immediate-break). `PYTHONPATH=. python3 -m pytest tests/test_presence_scheduler.py tests/test_scheduler_election.py tests/test_scheduler_jobs.py -v` → 52 passed (2 timer_reset skipped due to missing apscheduler). Commit `b7b2345`. Closes yebyen/mecris#316. **COMPLETE**.
...
- [ ] **Open PR yebyen:main → kingdonb:main** for all pending commits from sessions #64–#82 (narrator presence fix, NEON_DB_URL fix, upstream merge + legacy-cloud setup, CI/CD plan sync, ABI contract test x2, AGENTS.md sync, playwright fix, utcnow deprecation fix, async test fix, RAG test coverage, claude_monitor test coverage, billing_reconciliation test coverage, claude_monitor async path tests, get_reconciliation_summary tests, groq_odometer_tracker tests, mcp_reconcile_budget tests, claude_api_budget_scraper tests, scheduler background job tests, scheduler NameError fix, _start_leader_jobs tests). Closes yebyen/mecris#294, #295, #296, #298, #299, #302, #303, #305, #306, #307, #308, #309, #310, #311, #312, #313, #314, #315, #316.
...
- [ ] **Fix test_scheduler_jobs.py ordering dependency**: Tests in `test_scheduler_jobs.py` require `test_presence_scheduler.py` to run first (to prime sys.modules with mocked psycopg2). Add a conftest fixture or module-level sys.modules patch in `test_scheduler_jobs.py` so it can run in isolation.
...
- **scheduler test ordering (post-session #82)**: `test_presence_scheduler.py` must run before `test_scheduler_jobs.py`. Alphabetical pytest discovery (p < s) satisfies this naturally. Do not rely on isolated runs of `test_scheduler_jobs.py`.
