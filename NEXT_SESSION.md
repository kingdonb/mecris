...
- [x] **scheduler.py test coverage (session #80)**: `tests/test_scheduler_jobs.py` — 24 tests. Commit `a363bc0`. Closes yebyen/mecris#314. **COMPLETE**.
- [x] **claude_api_budget_scraper.py test coverage (session #79)**: `tests/test_claude_api_budget_scraper.py` — 24 tests. Commit `594b309`. Closes yebyen/mecris#313. **COMPLETE**.
- [x] **mcp_reconcile_budget.py test coverage (session #78)**: `tests/test_mcp_reconcile_budget.py` — 18 tests. Commit `2c19849`. Closes yebyen/mecris#312. **COMPLETE**.
...
- [ ] **Open PR yebyen:main → kingdonb:main** for all pending commits from sessions #64–#85 (narrator presence fix, NEON_DB_URL fix, upstream merge + legacy-cloud setup, CI/CD plan sync, ABI contract test x2, AGENTS.md sync, playwright fix, utcnow deprecation fix, async test fix, RAG test coverage, claude_monitor test coverage, billing_reconciliation test coverage, claude_monitor async path tests, get_reconciliation_summary tests, groq_odometer_tracker tests, mcp_reconcile_budget tests, claude_api_budget_scraper tests, scheduler background job tests, scheduler NameError fix, _start_leader_jobs tests, test_scheduler_jobs.py isolation fix, mcp_bridge tests, fetch_groq_usage tests). Closes yebyen/mecris#294, #295, #296, #298, #299, #302, #303, #305, #306, #307, #308, #309, #310, #311, #312, #313, #314, #315, #316, #317, #318, #319.
