...
- **PR open upstream**: kingdonb/mecris#178 from yebyen:main — full Rust service (Twilio Phase 2 + Phase 3 heuristics + Phase 3 I/O dispatch loop + OpenWeather check). Not yet merged.
...
- **yebyen/mecris ahead of kingdonb/mecris**: All divergence is in PR #178 + Phase 3 commits + OpenWeather commit.
- **All Rust crates pass locally**: sync-service now has 60 tests (was 52, +8 OpenWeather heuristic tests). Total across 6 crates: 94 tests, all green.
- **Phase 3 OpenWeather heuristic complete**: `is_weather_ok_for_walk` pure function + `fetch_weather_main` async + optional weather gate in dispatch loop. Commit `55a4e00`. Closes yebyen/mecris#152.
...
- [ ] **PR kingdonb/mecris#178 merged**: Python + Android are green (re-confirmed 2026-04-11 run 24288151090). Rust is a known pre-existing gap with documented fix in yebyen/mecris#142. Needs kingdonb review and merge. Note: PR head is now `55a4e00` (includes OpenWeather heuristic commit).
...
- [ ] **OpenWeather Spin variables not yet configured**: `openweather_api_key`, `openweather_lat`, `openweather_lon` must be added to Spin variables for the weather gate to activate. Without them, the dispatch loop skips the weather check gracefully (existing heuristics still apply).
- [ ] **spin.toml allowed_outbound_hosts**: The `fetch_weather_main` function calls `https://api.openweathermap.org` — this must be added to `allowed_outbound_hosts` for the `trigger-reminders` component in `spin.toml` before the weather check will work in live deployment.
