# Next Session: Check PR #159 merge status + begin Logic Vacuuming Phase 1
...
- **componentize-py research COMPLETE**: `docs/LOGIC_VACUUMING_CANDIDATES.md` updated with Candidate 3 (Python-native WASM via componentize-py). Committed as `b3db3f2`.
...
- [x] componentize-py research documented in yebyen/mecris#45 + LOGIC_VACUUMING_CANDIDATES.md.
- [x] Migration sequence updated: Phase 1.5 (arabic_skip_counter via componentize-py) added.
...
- **Logic Vacuuming Phase 1 decision**: Start ReviewPump WASM port. Choose path:
  - Option A (Python-native): `componentize-py` / `spin-python-sdk`. Validates Python-in-WASM pipeline. Preferred per kingdonb/mecris#157.
  - Option B (Rust): `cargo component` in `mecris-go-spin/review-pump/`. Already scaffolded.
  - Create a new plan issue for whichever is chosen.
- **Phase 1.5 candidate**: `arabic_skip_counter.py` — replace `psycopg2` with Neon HTTP API (`/sql` endpoint), then wrap with `componentize-py`. No Rust required. Validates I/O-via-HTTP pattern for Python WASM.
...
- Logic Vacuuming: ReviewPump is Phase 1 candidate. BudgetGovernor is Phase 2. Phase 1.5 is arabic_skip_counter via Python-native WASM. See `docs/LOGIC_VACUUMING_CANDIDATES.md`.
- `review-pump` WASM build (Rust path): `cargo build --target wasm32-wasip1 --release --features spin` in `mecris-go-spin/review-pump/`. Native unit tests: `cargo test` (no features needed). **Target `wasm32-wasip1` must be installed**: `rustup target add wasm32-wasip1`.
- **componentize-py path**: `fermyon/spin-python-sdk` + `componentize-py`. Pure Python services (no C extensions, no asyncio at WIT boundary). psycopg2 must be replaced with Neon HTTP API. Binary size ~10–30MB per component.
...
- **arabic_review_reminder**: Plan spec posted on yebyen/mecris#37. Phase 2 plan on yebyen/mecris#40. MCP wire-up on yebyen/mecris#41 (CLOSED). Phase 3 on yebyen/mecris#42 (CLOSED). skip_count_provider MCP wire-up on yebyen/mecris#43 (CLOSED). Sync PR plan on yebyen/mecris#44 (CLOSED). componentize-py research on yebyen/mecris#45 (CLOSED).
