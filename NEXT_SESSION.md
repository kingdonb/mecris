...
## Current Status (2026-04-27, post-session #61)
- **WASM test-implementation skew RESOLVED**: All 77 tests in `budget-governor-py` and `arabic-skip-counter` now pass. `budget-governor-py/app.py` pure-logic rewritten to match spec (4-bucket config, `_calc_total_spent`/`_calc_window_spent`, 39-min window, allow/defer/deny). `arabic-skip-counter/app.py` `_count_reminders` rewritten to sync httpx. Committed `ddac3b0`. Closes yebyen/mecris#290.
...
- **yebyen/mecris ahead of kingdonb/mecris by ~14 commits**: TF-IDF Search, Narrator enrichment, Observability Phase 1 (Python), Observability Phase 2 Python + Rust, notification_prefs read, JIT Secret Manager, notification_prefs write, SecretManager Neon fallback, WASM headless import guards, WASM test-implementation fixes. None yet PRed due to expired PAT.
...
- [x] **WASM test-implementation skew fix (session #61)**: `budget-governor-py/app.py` pure-logic section rewritten: `make_bucket_config(limits=None)` with 4-bucket defaults + type field; `_calc_total_spent` (all-time sum); `_calc_window_spent` (39-min rolling window, datetime-aware, skips invalid ts); `check_envelope` → allow/defer/deny; `recommend_bucket` prefers spend type; `get_status` returns envelope_status/window_minutes/envelope_spend_pct/recommendation; `budget_gate` returns budget_halted/envelope/message/routing_recommendation; `_parse_request` default cost 0.01; `_dump_spend_log_to_json` datetime encoder. `arabic-skip-counter/app.py`: `_count_reminders` rewritten as sync httpx.post against Neon HTTP SQL API; `_parse_query_params` guards against None. All 77 tests pass. Commit `ddac3b0`. **COMPLETE** — closes yebyen/mecris#290.
...
- [ ] **Open PR yebyen:main → kingdonb:main** for all pending commits (TF-IDF Search, Narrator enrichment, Observability Phase 1+2 Python, Observability Phase 2 Rust, notification_prefs read+write, JIT Secret Manager, SecretManager Neon fallback, WASM headless import guard, WASM test-implementation fixes) — blocked by expired PAT. Closes kingdonb/mecris#167, kingdonb/mecris#169, kingdonb/mecris#204, kingdonb/mecris#208 (complete), kingdonb/mecris#245 (Phase 1 + Phase 2 Python + Phase 2 Rust req 3).
