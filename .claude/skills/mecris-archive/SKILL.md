...
2. Rewrite `NEXT_SESSION.md` to reflect current reality
3. Append a dated entry to `session_log.md`
4. Commit both files with a consistent message
5. Close the plan issue with a completion comment
6. Stop
...
Assess honestly: did the work match the plan? Choose one: complete / partial / incomplete.
...
## Step 3 — Append to session_log.md

File lives at `session_log.md` in the repo root. Append (do not overwrite) a dated entry:
...
If `session_log.md` does not exist yet, create it with a header line then append the entry.

## Step 4 — Commit

```
git add NEXT_SESSION.md session_log.md
...
**Verify the commit succeeded** — run `git log --oneline -1` and confirm the commit hash appears before proceeding.

## Step 5 — Close the Plan Issue

Only after the commit is confirmed: find the open `[plan]` issue created this session and close it with a comment:
...
An open plan issue signals that archiving failed — this is intentional. If the issue is still open, the next session knows to re-archive.
